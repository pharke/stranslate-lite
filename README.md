...
TOML 单文件，保存即生效（下次触发时自动读取）。完整示例由 `stranslate-lite config --init` 生成：
...
```bash
stranslate-lite run                  # 启动后台工具（热键 + 悬浮窗 + 菜单栏图标）
stranslate-lite translate <文本>      # 一次性调用，结果打印 stdout
    --prompt <名称> | --hotkey <序号> | --stdin | --no-stream
stranslate-lite check [--ping]       # 配置校验 + 权限检查 + API 连通测试
...
```bash
pip install -e '.[dev]'
pytest            # 44 项：SSE 解析、重试、取消、取词判定、CLI 端到端（模拟服务端）等
```
...
- **macOS 适配器**在本仓库开发环境（Linux）无法运行验证，已按官方 API 语义编写并通过语法检查；请在 MacBook 上按此清单验收：
  1. `stranslate-lite check` 配置通过；`--ping` 连通；
  2. `run` 后按 `Alt+Q`（选中文本时）→ 悬浮窗流式出现回答；
  3. 无选中文本 → 悬浮窗提示取词失败；
  4. Esc / 关闭按钮可关窗；悬浮窗不抢走当前应用焦点；
  5. 菜单栏「译」→ 打开配置目录 / 检查权限 / 退出；
  6. 未授权辅助功能时，`check` 给出明确提示。
  若热键注册失败（OSStatus ≠ 0），通常是被其他应用占用，换个组合键即可。
